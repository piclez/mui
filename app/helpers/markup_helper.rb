module Merb::MerbInterface::MarkupHelper
  ...
  def mi_assets(options={})
    ...
    assets = ''
    if options[:css] == true
      assets << %{<link type="text/css" charset="utf-8" href="#{url(:merb_interface_stylesheets)}" media="all" rel="Stylesheet"/>}
    end
    if options[:js] == true
      assets << %{<script type="text/javascript" src="#{url(:merb_interface_javascripts)}"></script>}
    end
    assets
  end
  ...
  def mi_field(text, options={})
    ...
    attributes_input[:name] << @object if @object
    attributes_input[:name] << %{_#{options[:name]}} if options[:name]
  ...
  def mi_form(object, options={}, &block)
    attributes={}
    attributes[:action] = options[:url]
    attributes[:method] = 'post'
    @object = object
    %{<form #{attributes.to_xml_attributes}/>#{capture(&block)}</form>}
  end
...
include Merb::MerbInterface::MarkupHelper