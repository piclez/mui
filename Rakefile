...
mui_spec = Gem::Specification.new do |s|
  ...
  gems.each do |gem|
    s.add_dependency gem, ['>= #{VERSION}', '<= 1.0']
  end
end
...
task :install => [:install_gems, :package] do
  sh %{#{sudo} gem install #{install_home} --local pkg/mui-#{Mui::MORE_VERSION}.gem --no-update-sources}
end
...
task :bundle => [:package, :build_gems] do
  ...
  gems.each do |gem|
    sh %{cp #{gem}/pkg/#{gem}-#{VERSION}.gem bundle/}
  end
end
...
namespace :release do
  ...
  task :mui => [ :package ] do
    ...
    begin
      sh %{rubyforge login}
      sh %{rubyforge add_release #{NAME} mui #{VERSION} #{packages.join(' ')}}
      sh %{rubyforge add_file #{NAME} mui #{VERSION} #{packages.join(' ')}}
    rescue Exception => e
      puts
